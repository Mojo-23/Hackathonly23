...
- **PHASE-UI-000 ✅:** tokens + fonts + primitives — `globals.css` moved to `DESIGN_SYSTEM.md` §D's white-ground palette, General Sans + Inter self-hosted, existing primitives updated to the new tokens.
- **Public experience redesign ✅ (adopted):** the full public-facing experience — landing (`/`), `/organizers`, `/participants`, `/blog`, `/events`, `/events/[slug]`, and the shared header/footer — was designed and implemented directly by Claude as Design Authority (per `CLAUDE.md`), studying TAIKAI as a structural/craftsmanship reference (`docs/research/`) without copying its branding, assets, or content. This is the **official, adopted Hackathonly public design** — see `handoff/CLAUDE_IMPLEMENTATION_SUMMARY.md` for the authoritative build record. Future public-UI work evolves this implementation rather than reverting to the earlier, flatter design.
- **Next planned phase — Role-Aware Authentication and Dashboard Architecture (not yet scoped into a task):** separate Participant and Organizer account experiences, chosen at signup/onboarding, each with its own Dashboard; anonymous visitors never see a Dashboard link. This is backend/infrastructure work — database schema, migrations, RLS, route protection, and redirects — and follows the standing Claude-plans/Codex-implements/Claude-reviews workflow, not direct implementation. See the "Next Phase — Role-Aware Authentication and Dashboard Architecture" section of `handoff/CLAUDE_IMPLEMENTATION_SUMMARY.md` for the full requirements list.
- **Any future surface-by-surface UI task** still names `DESIGN_SYSTEM.md` as design authority and is reviewed against the closed sets and anti-slop doctrine.
